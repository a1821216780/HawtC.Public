...
{
    public class Snel:I_ALCal3DRotationalEffect
    {
        private Airfoil1 airfoil;
        private Vector<double> cl_0;
        private Vector<double> cla;
        public Snel(Airfoil1 air)
        {
            LogHelper.WriteLog("Run Snel Three-D Rotationa Effect Modol to Solve！", title: "[success]", leval: 1);
            Debug.WriteLine("AirFoilINI com");
            airfoil = air;
            cl_0 = LinearAlgebraHelper.zeros(airfoil.SecDataSet.Length);
            cla = LinearAlgebraHelper.zeros(airfoil.SecDataSet.Length);
            //#依据叶片翼型计算需要的参数
